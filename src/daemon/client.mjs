...
import { readDaemonInfo } from "./server.mjs";
 ...
 */
export function callDaemon(cacheDir, payload, { connectMs = 250, respMs = 2000 } = {}) {
...
export async function daemonAlive(cacheDir) {
...
export async function ensureDaemon(cacheDir) {
  if (await daemonAlive(cacheDir)) return true;
  const bin = path.join(path.dirname(fileURLToPath(import.meta.url)), "..", "..", "bin", "recollect.mjs");
  try {
